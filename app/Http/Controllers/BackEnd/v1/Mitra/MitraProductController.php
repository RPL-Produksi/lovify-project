...
namespace App\Http\Controllers\BackEnd\v1\Mitra;
...
class MitraProductController extends Controller
{
    ...
    public function storeProduct(Request $request, $id = null)
    {
        ...
        if ($id != null) {
            $product = Product::find($id)->where('mitra_id', Auth::user()->id)->first();
        ...
        if ($request->hasFile('cover')) {
            ...
            $storedFile = $file->storeAs('products' . '/' . $input['slug'] . '/' . Auth::user()->username . '/' . 'cover', $file->hashName());

    ...
    <script>
        $(document).ready(function() {
            let table = $('#kategoriTable').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: '{{ route('superadmin.kelola.admin.data') }}',
                    type: 'GET'
                },
                columns: [
                            ...
                            return `
                                <td class="d-flex d-xl-block">
                                    <button class="btn text-white edit-btn" data-target="#editAdminModal" data-toggle="modal"  style="background-color: #3D0A05" 
                                        data-id="${row.id}" data-fullname="${row.fullname}" data-username="${row.username}" 
                                        data-email="${row.email}" data-phone_number="${row.phone_number}">
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            ...
                            error: function(xhr) {
                                Swal.fire("Error!", xhr.responseJSON.message || "Gagal menghapus admin.", "error");
                            }
                        });
                    }
                });
